...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant.container and type(self.ant.ant)==QueenAnt and self.ant.ant.times_called>1:
                self.ant.ant = None
            else:
                assert self.ant == insect, '{0} is not in {1}'.format(insect, self)
                if insect.container and insect.ant is not None:
                    self.ant = insect.ant
                elif type(self.ant) == QueenAnt and self.ant.times_called == 1:
                    return
                else:
                    self.ant = None
        else:
...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
        if self.container == True and self.ant is None and other_ant.container == False:
            return True
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while location is not hive:
            if location.bees != [] and count in range(self.min_range, self.max_range):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        self.armor -= amount
        if self.armor <= 0:
            new_bee_list = list(self.place.bees)
            [fire.reduce_armor(self.damage) for fire in new_bee_list]
            self.place.remove_insect(self)

class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that can shoot in water as well as on land"""
    name = 'Scuba'
    implemented = True
    food_cost = 5
    watersafe = True
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        new_bee_list = list(self.place.bees)
        if len(new_bee_list) > 0:
            [ninja.reduce_armor(self.damage) for ninja in new_bee_list]
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        while entrance != colony.hive:
            if entrance.ant is not None and type(entrance.ant) is not QueenAnt:
                self.list_of_ants = self.list_of_ants + [entrance.ant]
            entrance = entrance.entrance
        while exit is not None:
            if exit.ant is not None:
                self.list_of_ants = self.list_of_ants + [exit.ant]
            exit = exit.exit
        for ant in self.list_of_ants:
            if ant not in self.double_ants:
                if ant.container == True and ant.ant != None and type(ant.ant) is not QueenAnt:
                    ant.ant.damage = 2*ant.ant.damage
                    self.double_ants+= [ant]
